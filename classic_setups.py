...
def _base_state(*, passed: bool, name: str, status: str, direction: str | None = None,
                signal_date=None, trigger_date=None, entry: float | None = None,
                stop: float | None = None) -> dict:
    return {
        "passed": bool(passed), "name": name, "status": status,
...
def _pfr_signal(df: pd.DataFrame, i: int, side: str, lookback: int = 2) -> bool:
    if i < lookback:
        return False
    if side == "buy":
        return float(df["Low"].iloc[i]) < float(df["Low"].iloc[i-lookback:i].min()) and float(df["Close"].iloc[i]) > float(df["Close"].iloc[i-1])
    return float(df["High"].iloc[i]) > float(df["High"].iloc[i-lookback:i].max()) and float(df["Close"].iloc[i]) < float(df["Close"].iloc[i-1])
...
def _pfr_state(df: pd.DataFrame, side: str) -> dict:
    ...
    if _pfr_signal(df, last, side):
        ...
        return _base_state(passed=True, name=name, status="Candle-sinal formado; aguardando o próximo candle", direction=direction,
                           signal_date=df.index[last], trigger_date=df.index[last], entry=entry, stop=stop)
    previous = last-1
    if previous >= 2 and _pfr_signal(df, previous, side):
        ...
        broke = float(df["High"].iloc[last]) > trigger if side == "buy" else float(df["Low"].iloc[last]) < trigger
        return _base_state(passed=broke, name=name, status="Entrada acionada no último candle" if broke else "Gatilho não rompido no candle seguinte",
                           direction=direction, signal_date=df.index[previous], trigger_date=df.index[previous], entry=trigger, stop=stop)
...
def _setup_123_signal(df: pd.DataFrame, i: int, side: str) -> bool:
    ...
    c1, c2, c3 = i-2, i-1, i
    if side == "buy":
        return float(df["Low"].iloc[c2]) < float(df["Low"].iloc[c1]) and float(df["Low"].iloc[c2]) < float(df["Low"].iloc[c3]) and float(df["Close"].iloc[c3]) > float(df["Open"].iloc[c3])
    return float(df["High"].iloc[c2]) > float(df["High"].iloc[c1]) and float(df["High"].iloc[c2]) > float(df["High"].iloc[c3]) and float(df["Close"].iloc[c3]) < float(df["Open"].iloc[c3])
...
def _setup_123_state(df: pd.DataFrame, side: str) -> dict:
    ...
    if _setup_123_signal(df, last, side):
        ...
        return _base_state(passed=True, name=name, status="Padrão 123 formado; aguardando rompimento do candle 3", direction=direction,
                           signal_date=df.index[last], trigger_date=df.index[last], entry=entry, stop=stop)
    previous = last-1
    if previous >= 2 and _setup_123_signal(df, previous, side):
        ...
        stop = float(df["Low"].iloc[c2] if side == "buy" else df["High"].iloc[c2])
        broke = float(df["High"].iloc[last]) > trigger if side == "buy" else float(df["Low"].iloc[last]) < trigger
        return _base_state(passed=broke, name=name, status="Entrada acionada no último candle" if broke else "Padrão formado, mas gatilho ainda não acionado",
                           direction=direction, signal_date=df.index[previous], trigger_date=df.index[previous], entry=trigger, stop=stop)
...
def _rolling_trigger_state(df: pd.DataFrame, ema9: pd.Series, signal_idx: int, side: str, name: str,
                           initial_stop: float | None = None) -> dict:
    direction = "Compra" if side == "buy" else "Venda"
    last = len(df)-1
    trigger_idx = signal_idx
    trigger = float(df["High"].iloc[signal_idx] if side == "buy" else df["Low"].iloc[signal_idx])
    stop = float(initial_stop if initial_stop is not None else (df["Low"].iloc[signal_idx] if side == "buy" else df["High"].iloc[signal_idx]))
    if signal_idx == last:
        return _base_state(passed=True, name=name, status="Candle-sinal formado", direction=direction,
                           signal_date=df.index[signal_idx], trigger_date=df.index[trigger_idx], entry=trigger, stop=stop)
    for j in range(signal_idx+1, len(df)):
        broke = float(df["High"].iloc[j]) > trigger if side == "buy" else float(df["Low"].iloc[j]) < trigger
        if broke:
            return _base_state(passed=j == last, name=name, status="Entrada acionada no último candle" if j == last else "Entrada já acionada anteriormente",
                               direction=direction, signal_date=df.index[signal_idx], trigger_date=df.index[trigger_idx], entry=trigger, stop=stop)
        if not _ema_direction_ok(ema9, j, side):
            return _base_state(passed=False, name=name, status=f"MME9 perdeu a inclinação de {direction.lower()} antes da entrada", direction=direction,
                               signal_date=df.index[signal_idx], trigger_date=df.index[trigger_idx], entry=trigger, stop=stop)
        trigger_idx = j
        trigger = float(df["High"].iloc[j] if side == "buy" else df["Low"].iloc[j])
        stop = min(stop, float(df["Low"].iloc[j])) if side == "buy" else max(stop, float(df["High"].iloc[j]))
    return _base_state(passed=True, name=name, status="Aguardando rompimento", direction=direction,
                       signal_date=df.index[signal_idx], trigger_date=df.index[trigger_idx], entry=trigger, stop=stop)
...
def _setup_93_signal(df: pd.DataFrame, ema9: pd.Series, i: int, side: str) -> bool:
    ...
    if side == "buy":
        return float(df["Close"].iloc[first]) < float(df["Close"].iloc[ref]) and float(df["Close"].iloc[second]) < float(df["Close"].iloc[first]) and float(df["Close"].iloc[first]) >= float(df["Low"].iloc[ref])
    return float(df["Close"].iloc[first]) > float(df["Close"].iloc[ref]) and float(df["Close"].iloc[second]) > float(df["Close"].iloc[first]) and float(df["Close"].iloc[first]) <= float(df["High"].iloc[ref])
...
def _setup_93_state(df: pd.DataFrame, ema9: pd.Series, side: str) -> dict:
    ...
    first = signal_idx-1
    initial_stop = min(float(df["Low"].iloc[first]), float(df["Low"].iloc[signal_idx])) if side == "buy" else max(float(df["High"].iloc[first]), float(df["High"].iloc[signal_idx]))
...
def _ifr2_stormer_state(df: pd.DataFrame, rsi2: pd.Series) -> dict:
    ...
    return _base_state(passed=True, name=name, status=f"IFR2 em {value:.2f}; entrada clássica no fechamento", direction="Compra",
                       signal_date=df.index[-1], trigger_date=df.index[-1], entry=close, stop=stop)
...
def _inside_bar_state(df: pd.DataFrame) -> dict:
    ...
    if len(df) < 2:
        return _base_state(passed=False, name=name, status="sem dados suficientes")
    inside = float(df["High"].iloc[-1]) < float(df["High"].iloc[-2]) and float(df["Low"].iloc[-1]) > float(df["Low"].iloc[-2])
    return _base_state(passed=inside, name=name, status="Inside Bar formado" if inside else "candle atual não é Inside Bar",
                       signal_date=df.index[-1] if inside else None, trigger_date=df.index[-1] if inside else None)
...
def special_setup_state(df: pd.DataFrame, op: str, left: pd.Series) -> dict:
    ...
    if op == "setup_123_sell": return _setup_123_state(df, "sell")
    if op == "setup_92_buy": return _setup_92_state(df, left, "buy")
    if op == "setup_92_sell": return _setup_92_state(df, left, "sell")
    if op == "setup_93_buy": return _setup_93_state(df, left, "buy")
    if op == "setup_93_sell": return _setup_93_state(df, left, "sell")
...
def evaluate_context_filter(df: pd.DataFrame, name: str) -> tuple[bool, str]:
    ...
    if name == "Inside Bar atual":
        if len(df) < 2: return False, "Inside Bar: sem dados suficientes"
    ...
    if name in {"Éden dos Traders — Compra", "Éden dos Traders — Venda"}:
        e8, e80 = ema(close, 8), ema(close, 80)
